...
class ListTasksHandler(private val storage: TaskStorage): HttpHandler {
    ...
    fun validatePage(page: Int, recordsPerPage: Int){
        if (page <= 0 ){
    ...
    fun getPageContents( page: Int, recordsPerPage: Int): String{
    ...
    override fun invoke(request: Request): Response {
        try{
           ...
           return  Response(Status.BAD_REQUEST).header("Content-type", "application/json").body(generateErrorBody("Ожидалось натуральное число в параметре page"))
        }
        catch (e: IllegalArgumentException){
            return  Response(Status.BAD_REQUEST).header("Content-type", "application/json").body(generateErrorBody(e.message ?: ""))
        }
    }
